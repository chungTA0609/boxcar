...
export default function EditCars() {
  ...
  const editCarData = useStoreState((state) => state.editCarData);
  const navigate = useNavigate(); // Use useNavigate for navigation in v6
  const userData = useStoreState((state) => state.userData);

  const [images, setImages] = useState([...editCarData.images]);
  const [imagesBinanry, setImagesBinanry] = useState([...editCarData.images]);
  ...
  const [params, setParams] = useState({
    ...
    manufacturingYear: editCarData.manufacturingYear,
    seatCapacity: editCarData.seatCapacity,
    status: editCarData.status,
    transmission: editCarData.transmission,
    drivetrain: editCarData.drivetrain,
    images: [...editCarData.images],
    slug: editCarData.slug,
    version: editCarData.version,
    kmDriven: editCarData.kmDriven,
    price: editCarData.price / 1000000,
    logo: editCarData.logo,
    brandId: editCarData.brand.id,
    styleId: editCarData.style.id,
    originId: editCarData.origin.id,
    fuelId: editCarData.fuel.id,
    outsideColorId: editCarData.outsideColor.id,
    insideColorId: editCarData.insideColor.id,
    cityId: "",
    districtId: "",
    wardId: "",
  ...
  const metadata = {
    title: "Chỉnh sửa tin bán xe",
    description: "Chỉnh sửa tin bán xe",
  };
  ...
  useEffect(() => {
    if (!isLogin || !userData.id) navigate("/login");
    getAllBrand();
    getAllCities();
    getAllFuel();
    getAllOrigin();
  ...
  useEffect(() => {
    if (editCarData && modelList.length) {
      const modelId =
        modelList.find((model) => model.name === editCarData.name)?.id || "";
  ...
  }, [modelList, editCarData, params.modelId]);
  useEffect(() => {
    if (params.cityId)
  ...
  const upLoadProcess = async () => {
    for (const element of imagesBinanry) {
      if (typeof element === "object") await uploadImg(element);
    }
  };

  const pushCar = async () => {
    try {
      await axiosInstance.put(`/cars/${editCarData.id}`, {
        ...params,
          ...
          <div className="content-column">
            <div className="inner-column">
              <div className="list-title">
                <h3 className="title">Chỉnh sửa tin bán xe</h3>
                <div className="text">
